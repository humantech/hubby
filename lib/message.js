...
var Message = function Message(uuid, client) {
	...
	Object.defineProperty(this, "content", {
		...
		set: function (v) {
			...
			var self = this;
			try{
				hubby().requestSchema(this.__schema.name, function(schema){
					try{
...
var extensions = {
	...
	load : function(fn) {
		var self = this;
		if (   !_.isNull(self.uuid)
			&& !_.isUndefined(self.uuid)
			&& !_.isUndefined(self.__client) 
			&& !_.isNull(self.__client)) {
			self.client.collection('messages', function(err, collection) {
				collection.find({uuid:self.uuid}, function(err, result) {
					if (!err && result) {
						console.log(self.uuid, " found");
						...
						console.log(self.uuid, " not found");
					}
				}); 
			});
		}
