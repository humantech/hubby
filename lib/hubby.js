...
var hubby = function hubby() {
	...
	var __saveMessage = function(message) {
	...
	var __createInstance = function() {
		return {
			...
			* if name was not founded, create a schema using schema 
			* parameter.
			*
			* @param name {String} [required] Schema name
			* @param fn {Function} [required] Callback
			* @param schema {Object} structure based on JSON Schema Internet Draft
			* @param version {Integer} Version control for Schemas
			*/
			requestSchema : function(name, fn, version, schema) {
				__checkInitialized();
			...
			requestMessage : function(uuid,fn) {
				...
				if (uuid) {
					...
					message.load(function(){ fn(message); });
				}
			},
			enqueue : function(message) {
				__checkInitialized();
				message.type = 'queue';
				__saveMessage(message);
			},
			...
			initialize : function(config) {
				...
				__redisClient.on('message',function(channel, message){
					...
					if (message === 'queued') {
						__redisQueue.lpop(channel, function(err, resp) {
							if (!err && resp) {
								__events.emit(message, resp);
							}
						});
					}
				});

				__redisClient.subscribe('queue01');
				...
				__mongo.open(function(err, client) {
					...
					__client.collection('messages', function(err, collection) {
						collection.find({status: 0}, {tailable : true, await_data: true}, function(err, cursor) {
							cursor.each(function(err, data) {
								if (!err && data) {

									console.log(JSON.stringify(data));

									var notification = {uuid: data.uuid};

									var message = new Message(data.uuid, __client);
									...
									message.load(function(){
										...
										if ( data.type === 'broadcast'){
											// message via EventEmmiter
											__events.emit('broadcast', notification);
										}
										if ( data.type === 'queue' ){
											// message via redis pubsub
											__redisQueue.rpush('queue01', JSON.stringify(notification), function(err, val) {
												if (!err) {
													__redisQueue.publish('queue01', 'queued');
												}
											});
										}
									});
...
hubby().on('initialized', function(){
	hubby().requestSchema('bounce', function(schema){
		console.log('founded schema: ' + schema.name + ' ' + schema.version);
	});
});

hubby().initialize();
setTimeout(function() {
	console.log('isInitialized ' + hubby().isInitialized());
}, 2000);
*/
