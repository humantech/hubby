...
var hubby = function hubby() {
	...
	var __popAndEmit = function(queue) {
		__redisQueue.lpop(queue, function(err, resp) {
			if (!err && resp) {
	...
	var __clearQueue = function(queue) {
		__redisQueue.llen(queue, function(err, resp) {
			if (resp > 0) {
				__popAndEmit(queue);
				__clearQueue(queue);
			}
			...
			*/
			...
			requestMessage : function(uuid,fn) {
				...
				if (!_.isNull(uuid) && !_.isUndefined(uuid)) {
					var message = new Message(uuid, __client);
			...
			enqueue : function(queue, message) {
				...
				__saveMessage(message, function(){
					__redisQueue.rpush(queue, JSON.stringify({uuid: message.uuid}), function(err, val) {
						if (!err) {
							__redisQueue.publish(queue, 'queued');
						}
					});
				});
			...
			subscribe: function(queue, f){
				__clearQueue(queue);

				__redisClient.on('message',function(channel, message){
					if (channel === queue && message === 'queued') {
						__popAndEmit(queue);
					}
				});

				__redisClient.subscribe(queue);
				f();
			},
			on : function(c, f) {
				__events.on(c, f);
