...
var hubby = function hubby() {
	...
	var __check_mongo_collections = function() {
		__clientSearch.collectionNames(function(err, names) {
			...
			_.each(__collections, function(name) {
				if (!_.include(names, name)) {
					__clientSearch.createCollection(name, __options, function(err, collection) {
						if (err) {
							throw err;
						}
		...
		return {
			...
			listSchemas : function(fn, initialized) {
				...
				if(initialized){
					...
					__clientSearch.collection('schemas', function(err, collection) {
			...
			createSchema : function(name, fn, version, schema) {
				...
				if(typeof __storedSchemas[name] === 'undefined' ||  
					...
					var __schema = new Schema(__clientSearch, name, version);
			...
			requestMessage : function(uuid,fn) {
				...
				if (!_.isNull(uuid) && !_.isUndefined(uuid)) {
					var message = new Message(uuid, __clientSearch);
			...
			setMessageStatus : function(uuid,status,fn){
				...
				var message;
				if (!_.isNull(uuid) && !_.isUndefined(uuid)) {
					message = new Message(uuid, __clientSearch);
					message.load(function(){ 
			...
			enqueue : function(queue, message) {
				...
				__saveMessage(message, function(){
					__redisQueue.rpush(queue, JSON.stringify({uuid: message.uuid, schema: message.schema}), function(err, val) {
						if (!err) {
			...
			initialize : function(config) {
				...
				__mongoSearch = new mongodb.Db(mongoConf.db, new mongodb.Server(
					mongoConf.host,
					mongoConf.port,
					{}
				), mongoConf.args);

				__mongoSearch.open(function(err, client) {
					if (err) {
						throw err;
					}
					__clientSearch = client;
					__check_mongo_collections();
				});

				__mongoTailable = new mongodb.Db(mongoConf.db, new mongodb.Server(
					mongoConf.host,
					mongoConf.port,
					{}
				), mongoConf.args);

				__mongoTailable.open(function(err, client) {
					if (err) {
						throw err;
					}
					__clientTailable = client;

					__clientTailable.collection('messages', function(err, collection) {
			...
			subscribe: function(schemas, f){
				...
				__redisClient.subscribe(schemas.join(' '));
				f();
			},
			on : function(c, f) {
				__events.on(c, f);
...
hubby().on('initialized', function(){
	...
	hubby().createSchema('SendEmailCommand', function(schema){
		
		console.log('Schema was initialized: ' + schema.name);
		hubby().createSchema('EmailSentReport', function(schema){
			
			console.log('Schema was initialized: ' + schema.name);
			hubby().createSchema('EmailBounceReport', function(schema){
			
				console.log('Schema was initialized: ' + schema.name);
			
			}, null, emailBounceReportSchema);

		}, null, emailSentReportSchema);

	}, null, sendEmailCommandSchema);
});
